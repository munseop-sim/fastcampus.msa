package ms2709.payservice.money.adapter.axon.saga;
...
public class MoneyRechargeSaga {
    ...
    @StartSaga
    @SagaEventHandler(associationProperty = "rechargingRequestId") //associationProperty는 saga의 id를 지정하는 것
    public void handle(RechargingRequestCreatedEvent event) {
        log.info("RechargingRequestCreatedEvent Start saga recharging request event  : {}", event.toString());
        var rechargingRequestId = event.getRechargingRequestId();
...
//
//        // "충전" 요청 시작
//        // 뱅킹의 계좌등록여부 확인 (RegisteredBankAccount)
//        // CheckRegisteredBankAccountCommand --> Check Bank Account
//        // --> axon server -> Banking service -> Common
//
//        //기본적으로 axon framework에서 모든 aggregate의 변경은 aggregate단위로 되어야만 한다.
